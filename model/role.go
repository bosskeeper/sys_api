...
func (r *Role) RoleSave(db *sqlx.DB) (role_code string, err error){
	...
	r.ActiveStatus = 1
	sql := `insert into Role(RoleCode,RoleName,Description,ActiveStatus,CreatorId,CreateDateTime) values(?,?,?,?,?,?)`
	res, err := db.Exec(sql,r.RoleCode,r.RoleName,r.Description,r.ActiveStatus,r.CreatorId,r.CreateDateTime)
	if err != nil {
		fmt.Println(err)
		return "",err
	}
...
func (r *Role)RoleUpdate(db *sqlx.DB)(role_code string, err error){
	...
	sql := `update Role set RoleCode=?,RoleName=?,Description=?,ActiveStatus=?,EditorId=?,EditDateTime=? where Id=? `
	res, err := db.Exec(sql,r.RoleCode,r.RoleName,r.Description,r.ActiveStatus,r.EditorId,r.EditDateTime,r.Id)
...
func (r *Role)RoleDisable(db *sqlx.DB)(role_code string, err error){
	r.EditDateTime = time.Now().String()
	sql := `update Role set RoleCode=?,RoleName=?,ActiveStatus=?,EditorId=?,EditDateTime=? where Id=? `
	res, err := db.Exec(sql,r.RoleCode,r.RoleName,r.ActiveStatus,r.EditorId,r.EditDateTime,r.Id)
	if err != nil {
		fmt.Println(err)
		return "", err
	}
